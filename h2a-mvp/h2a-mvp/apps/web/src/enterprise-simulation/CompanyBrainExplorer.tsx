...
export function CompanyBrainExplorer({ scenario, playback, onInspect, onInteract }: Props) {
  ...
  const chooseDepartment = (next: Department | 'All') => { onInteract?.(); setDepartment(next); setPage(0); setSelectedId(null); setHover(null); };
  const selectMemory = (memory: SimMemory) => { onInteract?.(); setSelectedId(memory.id); setHover(null); };
  const back = () => { onInteract?.(); if (selectedId) setSelectedId(null); else chooseDepartment('All'); };
  ...
  return <section className="company-brain-explorer" data-testid="company-brain-explorer" data-level={department === 'All' ? 'company' : 'team'} aria-label="Hospital company brain explorer">
    ...
    <div className="brain-explorer-layout">
      <div className="brain-network-column">
        <section className="brain-network" aria-labelledby="brain-network-title">
          ...
          <div className="brain-network-canvas" data-testid="brain-network-canvas">
            ...
            }) : pageMemories.map((memory, index) => <button key={memory.id} className={`brain-graph-node brain-memory-node brain-node-${memory.status.toLowerCase()} ${selectedId === memory.id ? 'brain-node-selected' : ''}`} style={{ '--node-x': `${MEMORY_POSITIONS[index][0]}%`, '--node-y': `${MEMORY_POSITIONS[index][1]}%` } as CSSProperties} data-memory-id={memory.id} aria-label={`Inspect memory: ${memory.title}`} aria-pressed={selectedId === memory.id} onClick={() => selectMemory(memory)} onMouseEnter={() => setHover({ kind: 'memory', memory })} onMouseLeave={() => setHover(null)} onFocus={() => setHover({ kind: 'memory', memory })} onBlur={() => setHover(null)}><span className="brain-node-icon"><BookOpen aria-hidden="true" /></span><strong>{short(memory.title)}</strong><small>{memory.status === 'Proposed' ? 'Review pending' : memory.status} · {usage.get(memory.id) || 0} reuses</small></button>)}
      ...
      <aside ref={details} className="brain-inspector" aria-label="Knowledge details" data-testid="brain-knowledge-details" aria-live="polite">
        {selected ? <><header><span className="brain-overline">KNOWLEDGE RECORD</span><Status status={selected.status} /><h3>{short(selected.title)}</h3><small>{selected.id} · version {selected.version}</small></header><section><h4><ShieldCheck aria-hidden="true" />Review and authority</h4><dl><dt>Source team</dt><dd>{selected.department}</dd><dt>{selected.status === 'Published' ? 'Reviewed by' : 'Review owner'}</dt><dd><button onClick={() => inspect({ kind: 'human', id: selected.reviewer })}>{personName(scenario, selected.reviewer)}<ArrowRight aria-hidden="true" /></button></dd><dt>Recorded</dt><dd>{date(selected.created)}</dd><dt>Reusable</dt><dd>{selected.status === 'Published' ? 'Yes, within the allowed scope' : 'No, excluded from agent context'}</dd><dt>Reuse count</dt><dd>{usage.get(selected.id) || 0} retained tasks</dd></dl><div className="brain-scope"><LockKeyhole aria-hidden="true" /><div><strong>Allowed teams</strong><p>{selected.allowedTeams.join(' + ')}</p><small>This record supplies context. It does not grant permission to execute a new action.</small></div></div></section><section><h4><BookOpen aria-hidden="true" />Reviewed context</h4><p className="brain-context-copy">{selected.content}</p></section><section><h4><GitBranch aria-hidden="true" />Source and provenance</h4><ol className="brain-provenance"><li><Users aria-hidden="true" /><span>Accountable human<strong>{sourceTask ? personName(scenario, sourceTask.owner) : personName(scenario, selected.reviewer)}</strong></span></li><li><Brain aria-hidden="true" /><span>Bound agent<strong>{sourceAgent?.name || 'Source outside retained window'}</strong></span></li><li><FileText aria-hidden="true" /><span>Source task<strong>{sourceTask?.title || selected.task}</strong></span></li><li><CheckCircle2 aria-hidden="true" /><span>Memory outcome<strong>{selected.status === 'Published' ? 'Reviewed and published' : selected.status === 'Rejected' ? 'Rejected; reuse excluded' : 'Waiting for human review'}</strong></span></li></ol>{sourceTask ? <button className="brain-detail-link" onClick={() => inspect({ kind: 'task', id: sourceTask.id })}>Inspect full identity trace<ArrowRight aria-hidden="true" /></button> : <p className="brain-retention-note">This record is retained; the source task is outside the current playback window.</p>}</section><section className="brain-output"><h4><FileText aria-hidden="true" />Completed task output</h4>{sourceOutput ? <><span className="brain-output-label">Sample operational deliverable</span><h5>{sourceOutput.title}</h5><p className="brain-output-summary">{outputSummary}</p><p className="brain-output-hint">The full handoff includes recommended actions, review gates and the linked identity record.</p>{sourceTask && <button className="brain-detail-link" onClick={() => inspect({ kind: 'task', id: sourceTask.id })}>View complete deliverable<ArrowRight aria-hidden="true" /></button>}<button className="brain-download" onClick={() => saveText(sourceOutput.markdown, sourceOutput.filename)}><Download aria-hidden="true" />Download deliverable</button></> : <p className="brain-retention-note">{sourceTask ? 'The source task has not completed. Its final deliverable appears after execution and review.' : 'No completed source task is available in this retained view.'}</p>}</section></> : <><header><span className="brain-overline">KNOWLEDGE ASSURANCE</span><Brain className="brain-inspector-hero" aria-hidden="true" /><h3>Useful context. Accountable origins.</h3><p>Choose a team, then a memory record. Its reviewer, permitted reuse and original work stay connected.</p></header><section><h4><ShieldCheck aria-hidden="true" />What the CISO can establish</h4><ul className="brain-checklist"><li><CheckCircle2 aria-hidden="true" /><span>Who reviewed this knowledge before an agent reused it.</span></li><li><LockKeyhole aria-hidden="true" /><span>Which teams may use it—and which records remain excluded.</span></li><li><GitBranch aria-hidden="true" /><span>Which task produced it, and how often it was reused.</span></li><li><FileText aria-hidden="true" /><span>The completed task’s actual sample deliverable, not just a success badge.</span></li></ul></section><section><h4>Review queue</h4>{proposed.length ? proposed.slice(0, 3).map(memory => <button className="brain-queue-item" key={memory.id} onClick={() => selectMemory(memory)}><Clock3 aria-hidden="true" /><span>{short(memory.title)}<small>{memory.department} · {personName(scenario, memory.reviewer)}</small></span><ChevronRight aria-hidden="true" /></button>) : <p>There are no knowledge records awaiting review.</p>}</section><section><h4>Latest published record</h4>{published[0] ? <button className="brain-featured-record" onClick={() => selectMemory(published[0])}><BookOpen aria-hidden="true" /><strong>{short(published[0].title)}</strong><span>View knowledge and completed output<ArrowRight aria-hidden="true" /></span></button> : <p>No reviewed record has been published yet.</p>}</section><footer className="brain-inspector-note">Operational context only. No patient records or clinical decision-making authority are represented here.</footer></>}
      </aside>
    </div>
  </section>;
}
