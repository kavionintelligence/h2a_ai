...
export function CollaborationExplorer({ scenario, playback, onInspect, onInteract }: Props) {
  ...
  const clearDetail = () => { setSelectedEdge(null); setSelectedNode(null); setHoveredId(null); setFocusedId(null); };
  ...
  const selectEdge = (id: string) => { onInteract?.(); setSelectedEdge(id); setSelectedNode(null); };
  const selectNode = (item: GraphNode) => {
    ...
    onInteract?.(); setSelectedNode(item.id); setSelectedEdge(null);
  };
  ...
  return <section className="collab-explorer" aria-label="Company collaboration map" data-testid="collaboration-explorer" data-level={roomId ? 'room' : department ? 'team' : 'company'}>
    ...
    <div className="collab-layout"><div className="collab-map-column">
      <div className="collab-map-context"><span><span className="collab-eyebrow">{roomId ? 'Room relationships' : department ? 'Team relationships' : 'Organisation relationships'}</span><strong>{title}</strong></span>{roomId && chosen && <label>Trace task<select aria-label="Trace task in room" value={chosen.id} onChange={event => { onInteract?.(); setTaskId(event.target.value); clearDetail(); }}>{context!.tasks.slice().sort((a, b) => b.created.localeCompare(a.created)).map(t => <option key={t.id} value={t.id}>{t.id} · {t.stage} · {t.title}</option>)}</select></label>}</div>
      <div className="collab-canvas" data-level={roomId ? 'room' : department ? 'team' : 'company'}>
        <svg viewBox={`0 0 980 ${graph.height}`} className="collab-svg" role="group" aria-label={`${title} relationship map. Use Tab to inspect nodes and connections.`}>
          ...
          {graph.edges.map(e => { const source = graph.nodes.find(n => n.id === e.source)!; const target = graph.nodes.find(n => n.id === e.target)!; const d = connectionPath(source, target, e.curved); return <g key={e.id} className={`collab-edge tone-${e.tone} ${selectedEdge === e.id ? 'selected' : ''} ${roomId && !e.task ? 'background' : ''}`} role="button" tabIndex={0} aria-label={`${e.label}: ${e.status}. ${e.detail}`} data-edge-id={e.id} onClick={() => selectEdge(e.id)} onKeyDown={event => activate(event, () => selectEdge(e.id))} onMouseEnter={() => setHoveredId(e.id)} onMouseLeave={() => setHoveredId(current => current === e.id ? null : current)} onFocus={() => setFocusedId(e.id)} onBlur={() => setFocusedId(current => current === e.id ? null : current)}><title>{e.label}: {e.detail}</title><path className="collab-edge-hit" d={d}/><path className="collab-edge-visible" d={d} markerEnd={`url(#${graphId}-${e.tone})`} strokeDasharray={e.tone === 'neutral' || e.tone === 'attention' ? '5 5' : undefined}/></g>; })}
          {graph.nodes.map(n => { const Icon = icons[n.kind]; const width = n.width || 180; return <g key={n.id} className={`collab-node ${n.kind} tone-${n.tone || 'neutral'} ${selectedNode === n.id ? 'selected' : ''}`} role="button" tabIndex={0} aria-label={`${n.kind === 'team' ? 'Open team' : n.kind === 'room' && !roomId ? 'Open room' : 'Inspect'} ${n.label}. ${n.sub}`} data-node={n.id} data-team={n.department} data-room={n.kind === 'room' && !roomId ? n.id : undefined} transform={`translate(${n.x},${n.y})`} onClick={() => selectNode(n)} onKeyDown={event => activate(event, () => selectNode(n))} onMouseEnter={() => setHoveredId(n.id)} onMouseLeave={() => setHoveredId(current => current === n.id ? null : current)} onFocus={() => setFocusedId(n.id)} onBlur={() => setFocusedId(current => current === n.id ? null : current)}><title>{n.label} — {n.sub}</title><rect x={-width / 2} y="-34" width={width} height="68" rx="9"/><Icon x={-width / 2 + 13} y="-20" width="18" height="18" strokeWidth="1.7"/><text x={-width / 2 + 38} y="-6" className="collab-node-title">{compact(n.label, n.kind === 'team' ? 24 : 21)}</text><text x={-width / 2 + 13} y="17" className="collab-node-sub">{compact(n.sub, width > 185 ? 33 : 27)}</text><circle cx={width / 2 - 10} cy="-23" r="3" className="collab-state-dot" /></g>; })}
        </svg>
        {(hoverEdge || hoverNode) && <div className="collab-tooltip" role="status"><strong>{hoverEdge?.label || hoverNode?.label}</strong><span>{hoverEdge ? `${hoverEdge.status} · ${hoverEdge.scope}` : hoverNode?.sub}</span></div>}
        <div className="collab-map-list" aria-label={`${title} relationships list`}>{graph.nodes.filter(n => n.kind !== 'company').map(n => { const Icon = icons[n.kind]; return <button key={n.id} data-team={n.department} data-room={n.kind === 'room' && !roomId ? n.id : undefined} onClick={() => selectNode(n)}><Icon/><span><strong>{n.label}</strong><small>{n.sub}</small></span><ChevronRight/></button>; })}</div>
      </div>
