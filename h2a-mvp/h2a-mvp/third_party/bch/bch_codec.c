...
/**
 * get_ecc_bits - get the exact number of ECC bits
 * @bch: BCH control structure
 * @returns: exact number of ECC parity bits
 * 
 * Helper function for WebAssembly/JavaScript integration
 */
unsigned int get_ecc_bits(struct bch_control *bch)
{
    if (!bch)
        return 0;
    return bch->ecc_bits;
