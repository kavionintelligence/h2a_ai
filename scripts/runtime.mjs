...
export function pythonCommand() {
  const candidates = [process.env.BYOSYNC_PYTHON,
  ...
  for (const candidate of candidates.filter(Boolean)) {
    const result = spawnSync(candidate, ['-c', 'import fastapi, sqlalchemy, uvicorn, agent_census, yaml'], { cwd: census, windowsHide: true, stdio: 'ignore' });
    if (result.status === 0) return candidate;
  }
  throw new Error('Python dependencies are unavailable. Run npm run setup (or set BYOSYNC_PYTHON to the configured interpreter).');
}
